...
$memory=system("snmpwalk -v 2c -c $key $ip .1.3.6.1.4.1.2011.5.25.31.1.1.1.1.7 | grep -v 'INTEGER: 0'  | awk -F '=' '{print $2}' | awk -F ':' '{print $2}' | tr -d ' '");
  echo "<br />";
$sql=" UPDATE datacenter_switch_status SET submission_date='$shi',switch_type='$type', zhuangtai='a',switch_cpu='$cpu',switch_memory='$memory',switch_temp='$tmper',switch_name='$name',switch_up='$linetime' WHERE switch_status_id='$id';";
mysql_query($sql);
